module Blazer
  class Result
    ...
    def chart_data
      case chart_type
      when "line" then { datasets: line_data }
      when "line2" then { datasets: line2_data }
      when "pie" then { datasets: pie_data, labels: @rows.map{ |r| smart_value(0, r[0]) } }
      when "bar" then { datasets: bar_data }
      when "bar2" then { datasets: bar2_data }
      when "scatter" then { datasets: scatter_data, x: @columns[0], y: @columns[1] }
      end
    end
